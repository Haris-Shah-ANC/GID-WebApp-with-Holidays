...
export default function EffortsComponent(props) {
    ...
    const addEmployeeTaskEfforts = async (EffortsList) => {
        ...
        if (Array.isArray(EffortsList)) {
            payload["effort_data"] = EffortsList
        } else {
            let index = EffortsList
            payload["effort_data"] = [{ hour: listOfTaskEfforts[index].working_duration, working_date: listOfTaskEfforts[index].working_date }]
        }
    ...
    const onSubmitClick = () => {
        ...
        for (let i = 0, len = listOfTaskEfforts.length, text = ""; i < len; i++) {
            if (listOfTaskEfforts[i].hasOwnProperty('action')) {

...
export default function IconInput(props) {
    const {inputType, disable, error, errorStyle, className, value, onTextChange, onBlurEvent, placeholderMsg, icon, isRightIcon, iconStyle} = props
  ...
  return (
    <div className={tailwindMergedCSS}>
        <input 
            ...
            name={inputType}
            value={value} 
            className={`border-none w-full rounded-md ${inputType === "datetime-local" ? "" : "pr-10"} font-quicksand font-medium text-sm`} 

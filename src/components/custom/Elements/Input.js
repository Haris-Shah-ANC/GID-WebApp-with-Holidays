...
const Input = React.forwardRef(
  ({className='', border, size, leftIcon, rightIcon, type, ...rest }, ref) => {
    const sizes = {
    ...
    let inputClasses =
      sizes[size] +
      " w-full placeholder-blueGray-200 text-blueGray-700 relative bg-white rounded-md outline-none transition duration-200 ";
    inputClasses = borders[border] + " " + inputClasses;
    let leftAddon = null;
    let rightAddon = null;
    let wrapperClasses = "mb-0 pt-0";
    if (leftIcon) {
    ...
    return (
      <>
        <div className={wrapperClasses} ref={ref}>
          {leftAddon}
          {type && type === "textarea" ? (
            <textarea {...rest} type={type} className={inputClasses+className} />
          ) : (
            <input {...rest} type={type} className={inputClasses+className} />
          )}
          {rightAddon}
        </div>
      </>

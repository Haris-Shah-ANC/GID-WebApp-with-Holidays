...
export default function FileUploadButton(props) {
  ...
  const uploadEmployeeData = async (data) => {
    let response = await apiActionFormData({ url: getTheTimeSheetUploadUrl(), method: 'post', data: { "file": data } }, onError)
  ...
  return (
    ...
    <div className={`flex flex-col md:flex-row w-full justify-center items-center gap-1 ${file ? "text-black" : "text-gray-500"}`}>
      ...
      <p className={`justify-center text-center align-middle`}>{svgIcons(`ml-2 mr-1 self-center fill-black`,"attachment")}</p> {file ? file.name : "file name.."}
    </div>
  )
}
